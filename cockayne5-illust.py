...
def drawitile(polygon,p,label=""):
    ...
    for i in range(len(polygon)):
        if i==0:
            ...
            strokewidth(2)
        line(polygon[i].start[0]/1000,polygon[i].start[1]/1000,
             (polygon[i].start[0]+polygon[i].delta[0])/1000,(polygon[i].start[1]+polygon[i].delta[1])/1000)
    if len(polygon) == 5:
        strokewidth(0.5)
