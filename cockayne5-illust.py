...
def test_drawoutline(edgeowners):
    ...
    for edge in edgeowners:
        if edgeowners[edge][0] is None or edgeowners[edge][1] is None:
            line(
                edge[0][0] / 1000.,
                edge[0][1] / 1000.,
                (edge[0][0] + edge[1][0]) / 1000.,
                (edge[0][1] + edge[1][1]) / 1000.)
...
def drawitile(polygon, p, label=""):
    ...
    for i in range(len(polygon)):
        if i == 0:
            ...
            strokewidth(2)
        line(
            polygon[i].start[0] / 1000,
            polygon[i].start[1] / 1000,
            (polygon[i].start[0] + polygon[i].delta[0]) / 1000,
            (polygon[i].start[1] + polygon[i].delta[1]) / 1000)
    if len(polygon) == 5:
        strokewidth(0.5)
        line(polygon[2].start[0] / 1000, polygon[2].start[1] / 1000,
             polygon[4].start[0] / 1000, polygon[4].start[1] / 1000)
